...
import {
  ...
  deleteCategoryServ,
  updateCategoryServ,
} from "../../services/carrerCategory.services";
...
function CarrerCategoriesList() {
  ...
  const handleDeleteCategoryFunc = async (id) => {
    ...
    if (confirmed) {
      try {
        ...
        if (response?.data?.statusCode === 200) {
          toast?.success(response?.data?.message || "category delete succefully");
          handleGetCategoryFunc();
        }
      } catch (error) {
        toast.error(
  ...
  return (
    <div className="bodyContainer">
      <Sidebar selectedMenu="Carrer" selectedItem="Carrer Category" />
...
export default CarrerCategoriesList;
