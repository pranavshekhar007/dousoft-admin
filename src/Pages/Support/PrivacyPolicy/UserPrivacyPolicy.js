...
function UserPrivacyPolicy() {
  ...
  const getSupportDetailsFunc = async () => {
    try {
      ...
      if (response?.data?.data) {
        setContent(response.data.data.userPrivacyPolicy || "");
        contentRef.current = response.data.data.userPrivacyPolicy || "";
  ...
  const handleSupportUpdateDetails = async () => {
    ...
    setBtnLoader(true);
    try {
      const response = await updateSupportDetailsServ({ _id, userPrivacyPolicy: contentRef.current });
