...
function UserTermsAndCondition() {
  ...
  const getSupportDetailsFunc = async () => {
    try {
      ...
      if (response?.data?.data) {
        setContent(response.data.data.userTermsAndCondition || "");
        contentRef.current = response.data.data.userTermsAndCondition || "";
  ...
  const handleSupportUpdateDetails = async () => {
    ...
    setBtnLoader(true);
    try {
      const response = await updateSupportDetailsServ({ _id, userTermsAndCondition: contentRef.current });
